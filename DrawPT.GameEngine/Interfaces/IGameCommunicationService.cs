...
public interface IGameCommunicationService
{
    ...
    Task<PlayerPrompt> AskPlayerImagePromptAsync(Player player, int timeoutInSeconds);
    Task<PlayerAnswer> AskPlayerQuestionAsync(Player player, GameQuestion question, int timeoutInSeconds);
    Task<GameGamble> AskPlayerGambleAsync(Player player, GameQuestion question, int timeoutInSeconds);

<template>
  <div id="lights-container" class="lights-container" ref="hyperspeedRef"></div>
</template>

<script lang="ts">
import { defineComponent, ref, onBeforeMounted, onBeforeUnmount, watch } from 'vue'
...
class App {
  ...
  init() {
    ...
    this.onWindowResize() // Initial size sync
    this.tick()
  }
...
export default defineComponent({
  ...
  props: {
    effectOptions: {
      type: Object as () => Partial<HyperspeedOptions>,
      default: () => ({})
    }
  },
  setup(props) {
    const hyperspeedRef = ref<HTMLElement | null>(null)
      ...
      try {
        ...
        myApp.loadAssets().then(() => {
          ...
          if (currentRect.width > 0 && currentRect.height > 0) {
            ...
            console.warn('Container still has zero dimensions, initialization skipped')
          }
        })
      } catch (error) {
      ...
      if (typeof ResizeObserver !== 'undefined') {
        resizeObserver = new ResizeObserver((entries) => {
          for (const entry of entries) {
            ...
            if (width > 0 && height > 0 && !appRef.value) {
              initApp()
            }
          }
        })

        if (hyperspeedRef.value) {
          resizeObserver.observe(hyperspeedRef.value)
        }
      }
...
/* Ensure the canvas element itself takes full height */
